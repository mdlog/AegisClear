...
|---|---|---|---|---|
| Yul `poseidon-solidity` T3 | hash([1,2]) | 32.503 | **WAJIB** — menunggu deploy oleh user (§3, langkah 5) | == circomlibjs (D3 ✅) |
| Yul T6 | hash(5 input) | 172.418 | – (di luar gerbang D2; leaf-only, tidak ada langkah on-chain untuk ini) | leaf |
| Yul 7× T3 | anchored ack | 212.715 | opsional (disarankan) — menunggu deploy caller rantai-7 oleh user (§3, "Opsional") | dipakai hanya jika ingin `rasio_chain7` yang bersih on-chain |
| Stylus OZ Poseidon2 | hash([1,2]) | – | **WAJIB** — menunggu deploy oleh user; pakai panggilan **KEDUA/ter-cache** (§3, langkah 3) | Poseidon2 ≠ v1 |
| Stylus OZ Poseidon2 | hashChain7 | – | opsional, berpasangan dengan baris Yul 7× T3 — pakai panggilan **KEDUA/ter-cache** | |
