...
- [ ] **Step 1d: Result-table polish (controller browser check after Task 6 fix round)** — in `web/src/styles.css`: `.table.result { min-width: 880px; }` (the `.scroll` wrapper already scrolls horizontally) and `.txlink { display: inline-block; white-space: nowrap; margin-right: 6px; }` so a tx link never breaks mid-hash and rows stay compact; keep `.table.result td { white-space: normal }` for the text columns. Rebuild and eyeball `/` after a `all` run: 4 rows, each ≤ 3 lines tall, no overflow outside the panel. Also (re-review minor): in `PrivacyCards.tsx` reset `chs`/`err` synchronously at the start of the effect (`setChs([]); setErr(null);`) so a stale card/banner from the previous run never shows next to the new one; remove the redundant `table-layout: auto` declaration.
