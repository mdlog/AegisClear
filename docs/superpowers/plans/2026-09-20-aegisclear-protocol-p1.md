...
- [ ] **Step 4b: SDK robustness (Task 7 re-review carry-over)** — `sdk/src/provider/server.ts` `ensureModeMatchesFactory()`: on rejection clear the cached promise (`modeCheck = undefined` in a `.catch` that rethrows) so a transient RPC error does not wedge the provider forever; `sdk/src/client/agent.ts` `exitUnilateral()` error text → `cannot exit in state …`.
