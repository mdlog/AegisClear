...
new Watcher({ ctx: { publicClient, walletClient, factory: FACTORY as Address, chainId: CHAIN_ID }, log: console.log, intervalMs: 10_000 }).start();
console.log(`watcher: factory ${FACTORY} on ${CHAIN_ID} (settle/sweep only — no challenge responder; run startProviderWatcher in-process for that)`);
